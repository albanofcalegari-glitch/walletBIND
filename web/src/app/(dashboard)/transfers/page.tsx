...
export default function TransfersPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-slate-900 p-1 rounded-lg w-fit">
        {(['send', 'history'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm rounded-md font-medium transition-colors ${tab === t ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
          >
      ...
      {tab === 'send' ? <SendTransfer /> : <TransferHistory />}
    </div>
  );
}
