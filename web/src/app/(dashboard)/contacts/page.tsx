  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-surface-900 dark:text-white">Contactos Favoritos</h1>
          <p className="text-sm text-surface-500 mt-1">Guarda tus destinatarios frecuentes para transferir mas rapido</p>
        </div>
        <button
      ...
      <div className="space-y-3">
        {filtered.map((contact) => (
          <div key={contact.id} className="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-xl p-5 hover:border-surface-300 dark:hover:border-surface-700 transition-colors">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-brand-500/10 flex items-center justify-center text-brand-600 dark:text-brand-400 font-bold text-sm">
                  {contact.name.split(' ').map(w => w[0]).join('').slice(0, 2)}
                </div>
                <div>
                  <p className="text-surface-900 dark:text-white font-medium">{contact.name}</p>
                  <p className="text-xs text-surface-400 mt-0.5">@{contact.alias} &middot; {contact.bank}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleEdit(contact)}
                  className="p-1.5 text-surface-400 hover:text-surface-900 dark:hover:text-white rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <button
                  ...
                  className="p-1.5 text-surface-400 hover:text-danger-400 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="mt-3 flex items-center gap-6">
              <div>
                <p className="text-[10px] text-surface-400 uppercase">CBU</p>
                <p className="text-xs text-surface-500 font-mono">{contact.cbu}</p>
              </div>
              {contact.notes && (
                <div>
                  <p className="text-[10px] text-surface-400 uppercase">Nota</p>
                  <p className="text-xs text-surface-500">{contact.notes}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {filtered.length === 0 && (
          <div className="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-xl p-8 text-center text-surface-400">
  ...
  return (
    <div className="bg-white dark:bg-surface-900 border border-brand-500/30 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-surface-900 dark:text-white">{initial ? 'Editar contacto' : 'Nuevo contacto'}</h2>
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSave({ name, alias, cbu, bank, notes }); }} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-surface-500 mb-1">Nombre / Razon social</label>
          ...
          <div>
            <label className="block text-sm text-surface-500 mb-1">Alias</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-surface-500 mb-1">CBU / CVU</label>
            <input type="text" value={cbu} onChange={(e) => setCbu(e.target.value)}
              className="w-full px-4 py-2.5 bg-surface-100 dark:bg-surface-800 border border-surface-300 dark:border-surface-700 rounded-lg text-surface-900 dark:text-white placeholder-surface-400 focus:outline-none focus:ring-2 focus:ring-brand-500"
              placeholder="0000000000000000000000" required />
          </div>
          <div>
            <label className="block text-sm text-surface-500 mb-1">Banco</label>
        ...
        <div>
          <label className="block text-sm text-surface-500 mb-1">Nota (opcional)</label>
